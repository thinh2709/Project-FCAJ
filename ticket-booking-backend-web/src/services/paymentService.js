...
const queryMoMoTransaction = async (orderId) => {
  const requestId = orderId + '_query'; // Must be unique per query
...
const checkPaymentStatus = async (bookingId, userId = null, orderId = null) => {
  let queryText = `
    SELECT b.id, b.status, b.payment_status, b.payment_method, b.momo_transaction_id, b.total_amount
    FROM bookings b
    WHERE b.id = $1
  `;
  ...
  if (booking.status === 'pending' && booking.payment_method === 'momo' && orderId) {
    ...
    if (momoResult && momoResult.resultCode === 0) {
      ...
      const updated = await db.query('SELECT status, payment_status, momo_transaction_id FROM bookings WHERE id = $1', [bookingId]);
      if (updated.rows.length > 0 && updated.rows[0].status !== 'pending') {
        ...
        booking.momo_transaction_id = updated.rows[0].momo_transaction_id;
      } else {
        // If DB hasn't updated yet (async worker), optimistic return
        booking.payment_status = 'paid';
        booking.status = 'confirmed';
        booking.momo_transaction_id = momoResult.transId;
      }
    }
  }
  ...
  return {
    ...
    transactionId: booking.momo_transaction_id,
