 ...
 */
const getAutoConfig = async (eventId) => {
  try {
    const data = await redisReader.get(`queue:config:auto:${eventId}`);
    if (!data) return { enabled: false, maxUsers: 50 };
    return JSON.parse(data);
  } catch (error) {
    logger.error('Failed to get auto queue config', { eventId, error: error.message });
    return { enabled: false, maxUsers: 50 };
  }
};

const setAutoConfig = async (eventId, config) => {
  try {
    await redisPrimary.set(`queue:config:auto:${eventId}`, JSON.stringify(config));
    return config;
  } catch (error) {
    logger.error('Failed to set auto queue config', { eventId, error: error.message });
    throw error;
  }
};
