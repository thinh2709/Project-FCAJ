    ...
    setInterval(async () => {
      try {
        ...
        for (const match of matches.rows) {
          ...
          // 3. 1-In-1-Out logic: If no one is currently in the booking page, and there are waiting users
          if (stats.currentAllowedUsers === 0 && stats.waitingUsers > 0) {
            // Allow exactly 1 user in
            const allowed = await queueService.allowUsers(matchId, 1);
            if (allowed > 0) {
              logger.info(`[Auto-Queue] Allowed 1 next user from waitlist for match ${matchId}`);
            }
          }
        }
      } catch (err) {
